...
#MPR121 Touch Breakout
i2c = busio.I2C(board.SCL, board.SDA)
mpr121 = adafruit_mpr121.MPR121(i2c)

def setCapThresholds(touchThreshold = 12, releaseThreshold = 6):
    #Write to the reset register.
    mpr121._write_register_byte(adafruit_mpr121.MPR121_SOFTRESET, 0x63)
    time.sleep(
        0.001
    ...
    for i in range(12):
        mpr121._write_register_byte(adafruit_mpr121.MPR121_TOUCHTH_0 + 2 * i, touchThreshold)
        mpr121._write_register_byte(adafruit_mpr121.MPR121_RELEASETH_0 + 2 * i, releaseThreshold)
    
    # Enable all electrodes.
    mpr121._write_register_byte(
            adafruit_mpr121.MPR121_ECR, 0x8F
    )  # start with first 5 bits of baseline tracking

setCapThresholds(100, 6)

#END - MPR121 Touch Breakout
...
A1 = Button("A1", 1, [9], 0x101010,  lambda: mpr121[0].value)
B1 = Button("B1", 2, [9, 8], B1_Color,  lambda: mpr121[0].value and mpr121[1].value) #this "button" is pressed if both pins are pressed
A2 = Button("A2", 3, [8], 0x101010,  lambda: mpr121[1].value)
B2 = Button("B2", 4, [8, 7], B2_Color,  lambda: mpr121[1].value and mpr121[2].value)
A3 = Button("A3", 5, [7], 0x101010,  lambda: mpr121[2].value)
B3 = Button("B3", 6, [7, 6], B3_Color,  lambda: mpr121[2].value and mpr121[3].value)
A4 = Button("A4", 7, [6], 0x101010,  lambda: mpr121[3].value)
...
Buttons = [B1, B2, B3, A1, A2, A3, A4] #A list we can iterate through. Must Evaluate Double buttons first as they'll block single buttons.
...
def UpdateLightsToTouches():
    ...
    if (B1.Held() and B2.Held()) or (B2.Held() and B3.Held()) or (B3.Held()):
    ...
    while ble.connected:
        ...
        if cp.switch: # Do real functionality when switch is on
            if True: #MODE == 0:
                ...
                #B1 - Move Mouse Up
                if Buttons[0].state == ButtonStates.JUST_RELEASED_PRESS:
                ...
                #B2 - Tap to Left Click, Hold to Hold L-Mouse
                ...
                elif Buttons[4].state == ButtonStates.DOWN:
                    MOUSE_ACCEL_TIMER += MOUSE_ACCEL_SPEED
                    speed = clamp((1 + MOUSE_ACCEL_TIMER), 1, MOUSE_MAX_SPEED)
                ...
                #A4 - Scroll wheel, press to change directions (and scroll a little), hold to scroll
                    ...
                    if SCROLL_UP:
                        mouse.move(wheel=3)
                         ...
                         mouse.move(wheel=-3)
                    time.sleep(0.2)
                elif Buttons[6].state == ButtonStates.DOWN:
                    if SCROLL_UP:
                        mouse.move(wheel=1)
                    else:
                        mouse.move(wheel=-1)
            ...
            #TODO: Make a new mode and add a bunch of new functions. As well as more clear feedback we're in a new mode.
                ...
                True
                # HOLD LEFT MOUSE
                # elif Buttons[].state == ButtonStates.JUST_RELEASED_HOLD:
                ...
                # # - Cycle Open windows and on screen Keyboard
                ...
                # # - Right Click, also right click and drag.
                # elif Buttons[3].state == ButtonStates.JUST_RELEASED_PRESS:
                #     if RIGHT_MOUSE_HELD:
                #         mouse.release(Mouse.RIGHT_BUTTON)
                #         RIGHT_MOUSE_HELD = False
            ...
            #print(message)
            #print((cp._touches[0].raw_value, cp._touches[1].raw_value, cp._touches[2].raw_value, cp._touches[3].raw_value))

            #print(mpr121[0].threshold)

            print((mpr121[0].raw_value, mpr121[1].raw_value, mpr121[2].raw_value, mpr121[3].raw_value, mpr121[4].raw_value))
            #print((mpr121[0].value, mpr121[1].value, mpr121[2].value, mpr121[3].value, mpr121[4].value))
